...
def encode_set(values, vtype=None, is_enum_type=False):
    ...
    return "{" + ", ".join(encode_scalar(v, vtype=vtype, is_enum_type=is_enum_type) for v in sorted_values) + "}"


def encode_array(values, shape=None, scalar_formatter=None, vtype=None, is_enum_type=False):
    ...
    if (scalar_formatter is None):
        scalar_formatter = lambda value: encode_scalar(value, vtype=vtype, is_enum_type=is_enum_type)
